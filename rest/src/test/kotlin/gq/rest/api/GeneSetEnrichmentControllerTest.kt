...
open class GeneSetEnrichmentControllerTest {
    ...
    fun bigDecimalCloseTo(value: Double, error: Double) =
        BigDecimalCloseTo.closeTo(BigDecimal(value), BigDecimal(error))

    @Test
    fun testBasicRequestSameSpeciesEntrezToEntrez() {
        val requestForm = GeneSetEnrichmentController.EnrichmentRequestForm()
        val queryGenes = listOf(
            "494143", "390916", "375444", "153527", "139341", "112495", "91947", "91942", "80762",
        ...
        mockMvc.makeRequest(URL, requestForm)
            ...
            .andExpect(
                jsonPath(
                    "$.result.enrichmentResultItems[0].logAdjPvalue",
                    bigDecimalCloseTo(-131.88698551435778, 1e-5)
                )
            )
    ...
    fun testNetworkClustering() {
        val requestForm = GeneSetEnrichmentController.EnrichmentRequestForm()
        val queryGenes =
            repository.moduleCollection.fullNameToGQModule[Triple(10089, 96, 14)]!!.sortedEntrezIds.slice(0..70)
                .plus(
                    repository.moduleCollection.fullNameToGQModule[Triple(
                        10089,
                        96,
                        15
                    )]!!.sortedEntrezIds.slice(0..70)
                )
                .plus(
                    repository.moduleCollection.fullNameToGQModule[Triple(
                        10089,
        ...
        mockMvc.makeRequest(URL, requestForm)
            ...
            .andExpect(
                jsonPath(
                    "$.result.networkClusteringGroups.1.moduleNames",
                    equalTo(listOf("GSE10089_GPL96#14", "GSE10089_GPL96#15"))
                )
            )
