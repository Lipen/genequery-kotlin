...
import gq.core.gea.MinLogAdjPvalueScoreEvaluationStrategy
import gq.core.gea.ScoredEnrichedItemsGroup
import gq.core.gea.SpecifiedEntrezGenes
import gq.core.gea.findBonferroniSignificant
import gq.core.gea.groupEnrichedItemsByClusters
...
data class EnrichmentResponse(
    val identifiedGeneFormat: String,
...
open class GeneSetEnrichmentService @Autowired constructor(
    ...
    open fun findEnrichedModules(
        rawGenes: List<String>,
        speciesFrom: Species,
        speciesTo: Species = speciesFrom
    ): EnrichmentResponse {
        val (identifiedGeneFormat, conversionMap) = convertGenesToEntrez(
            rawGenes,
            speciesFrom,
            speciesTo,
            gqDataRepository
