...
class ResponseWrapper(
    @JsonInclude(JsonInclude.Include.NON_NULL) val result: Any?,
    val success: Boolean = true,
    @JsonInclude(JsonInclude.Include.NON_NULL) var errors: List<String>? = null
)

class SuccessResponseEntity(body: Any?) :
    ResponseEntity<ResponseWrapper>(ResponseWrapper(body, true, null), HttpStatus.OK)

class ErrorResponseEntity(errors: List<String>?, status: HttpStatus) :
    ResponseEntity<ResponseWrapper>(ResponseWrapper(null, false, errors), status) {
...
class RestControllerAdvice : ResponseBodyAdvice<Any?> {
    ...
    companion object {
        val LOG = LogManager.getLogger(GeneSetEnrichmentController::class.java)
    ...
    fun handleBadRequestException(e: BadRequestException): ErrorResponseEntity {
        ...
        return ErrorResponseEntity(
            e.cause?.message ?: e.message ?: HttpStatus.BAD_REQUEST.reasonPhrase,
