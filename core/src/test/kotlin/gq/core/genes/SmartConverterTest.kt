...
class SmartConverterTest {
    ...
    fun testSymbolToSymbol() {
        assertEquals(
            "A B C D".split(" "),
            smartConverter.toSymbol("a b c d".split(" "), GeneFormat.SYMBOL, Species.HUMAN).values.toList()
        )
        assertEquals(
            "A B C D".split(" "),
            smartConverter.toSymbol("a b c d".split(" "), GeneFormat.SYMBOL, Species.RAT).values.toList()
        )
        assertEquals(
            listOf("A1", "A3", null),
            smartConverter.toSymbol(
                "a1 a3 a4".split(" "),
                GeneFormat.SYMBOL,
                Species.RAT,
                Species.HUMAN
