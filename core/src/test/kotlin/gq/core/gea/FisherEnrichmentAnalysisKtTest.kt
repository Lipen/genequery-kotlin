...
class FisherEnrichmentAnalysisKtTest {
    ...
    companion object {
        var dataset = GQModuleCollection(
    ...
    fun readEntrezIds(localPath: String) =
        String(
            Files.readAllBytes(Paths.get(Thread.currentThread().contextClassLoader.getResource("gea/$localPath").toURI()))
