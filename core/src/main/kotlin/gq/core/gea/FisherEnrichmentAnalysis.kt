...
data class EnrichmentResultItem(
    val gse: Int,
    val gpl: Int,
    ...
    val moduleSize: Int
) : Comparable<EnrichmentResultItem> {
...
fun findBonferroniSignificant(
    ...
    bonferroniMaxPvalue: Double = 0.01
): List<EnrichmentResultItem> {
    val experimentsForThisSpecies = moduleCollection.speciesToGseGpl[query.species] ?: return emptyList()
    val queryEntrezIds =
        if (query.entrezIds.isNotEmpty()) query.entrezIds.toSortedSet().toLongArray() else return emptyList()
    ...
    return experimentsForThisSpecies.mapNotNull(
        fun(it: Pair<Int, Int>): List<EnrichmentResultItem>? {
            ...
            return modules.filter { it.number > 0 && moduleIdToIntersectionSize[it.number]!! > 0 }
                .mapNotNull { module ->
