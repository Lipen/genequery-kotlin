...
enum class GeneFormat(val formatName: String) {
    ENTREZ("entrez") {
        override fun isFormatOf(gene: String): Boolean = try {
            isFormatOf(gene.toLong())
    ...
    REFSEQ("refseq") {
        override fun isFormatOf(gene: String): Boolean =
            GeneFormat.REFSEQ_PREFIXES.any { isPrefixAndDigitsWithMaybeDot(gene, it) }

        override fun normalize(gene: String) = gene.toUpperCase().substringBefore(".")
    },
    ENSEMBL("ensembl") {
        override fun isFormatOf(gene: String): Boolean =
            GeneFormat.ENSEMBL_PREFIXES.any { isPrefixAndDigitsWithMaybeDot(gene, it) }
