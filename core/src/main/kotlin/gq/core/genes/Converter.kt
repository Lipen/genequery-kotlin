...
abstract class FromGeneToGeneConverter<TFrom, TTo : Any, TSelf : FromGeneToGeneConverter<TFrom, TTo, TSelf>>(
    ...
    fun convert(species: Species, fromIds: Iterable<TFrom>) =
        convertDetailed(species, fromIds).values.filterNotNull().toSet()
...
class ToEntrezConverter(entrezOtherMappings: Iterable<GeneMapping> = emptyList()) :
     ...
     */
    fun normalizeAndConvert(
        species: Species,
        geneIds: List<String>,
        format: GeneFormat = GeneFormat.guess(geneIds)
    ): Map<String, Long?> {
...
class SmartConverter(
    private val toEntrezConverter: ToEntrezConverter,
        ...
        return when (formatFrom) {
            GeneFormat.ENSEMBL, GeneFormat.REFSEQ -> {
                ...
                val entrezOriginalToTarget = orthologyConverter.getOrthologyByEntrez(
                    ensemblToOriginalEntrez.values.filterNotNull(), speciesTo
                ).mapValues { it.value?.entrezId }
            ...
            GeneFormat.SYMBOL -> {
                ...
                val normalizedIdsToOrthology =
                    orthologyConverter.getOrthologyBySymbol(normalizedIds.values, speciesTo)
                        .mapValues { it.value?.entrezId }
            ...
            GeneFormat.ENTREZ -> {
                ...
                val normalizedToOrthology = when (formatFrom) {
                    GeneFormat.ENSEMBL, GeneFormat.REFSEQ -> {
                        ...
                        crossLinkMap(
                            normalizedToEntrez,
                            orthologyConverter.getOrthologyByEntrez(
                                normalizedToEntrez.values.filterNotNull(),
                                speciesTo
                            )
                        )
                    }
                ...
                geneIds.associate { Pair(it, formatFrom.normalize(it)) }
                    .mapValues { normalizedToOrthology[it.value]?.symbolId }
            }
        }
    }
